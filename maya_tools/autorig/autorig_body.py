...
from .arm import create_clavicle_arm_setup
from .leg import create_hip_leg_setup
...
from ..curve import ensure_shape
...
def autorig_body(
    ...
    spine_matrix(proxy_root, proxy_chest)
    ...
    create_hip_leg_setup(proxy_root, proxy_leg_jnt_l, proxy_leg_jnt_r, geo_l, geo_r)


def run_autorig_body():
    ...
    matrix_constraint('bind_chest', 'ctrl_clavicle_L_move', mo = True, t = True, r = True)
    matrix_constraint('bind_chest', 'ctrl_clavicle_R_move', mo = True, t = True, r = True)
    matrix_constraint('bind_pelvis', 'ctrl_hips_move', t = True, r = True)

    cmds.parent('switch_leg_L', 'switch_leg_R', 'ctrl_root')
    cmds.parent('switch_arm_L', 'switch_arm_R','ctrl_ik_chest')

    offset_parent_matrix(['switch_leg_L', 'switch_leg_R', 'switch_arm_L', 'switch_arm_R'])
    cb_attributes(['switch_leg_L', 'switch_leg_R', 'switch_arm_L', 'switch_arm_R'], lock = True, hide = True)

    no_roll_locs('drv_leg_L', 'bind_hip_L', 'ctrl_A_ribbon_leg_L_01', invert = True)
    no_roll_locs('drv_leg_R', 'bind_hip_R', 'ctrl_A_ribbon_leg_R_01', invert = True)

    ensure_shape()
    vis_no_keyable('ctrl_main')

    sep_cb('ctrl_main', True)
