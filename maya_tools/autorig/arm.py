...
def create_fk_arm(proxy_arm_jnt: str):
    ...
    SIDE = get_side_from_node(proxy_arm_jnt)
...
def create_switch_node_arm(drv_jnts: list):
    ...
    SIDE = get_side_from_node(drv_arm)
    ...
    value = distance_btw(drv_arm, drv_elbow) * 0.5
...
def create_ik_setup_arm(drv_jnts: list):
    ...
    ctrl_ik = octagon_control(name = f'{CTRL}_arm_{SIDE}', normal = 'x', color = SIDE_COLOR[SIDE])
    ...
    cmds.move(0, 0, distance_btw(drv_arm, drv_elbow) * -1, pole_vector, relative = True)
...
def create_ik_fk_setup_arm(proxy_arm_jnt: str):
    '''
    '''
    ...
    switch_ik_fk(fk_joints, drv_joints, switch_node = switch_node, ctrl = ctrl_ik, pv = pv)

    ctrl_start_a, ctrl_end_b, ribbon_assemble_shape = limb_ribbon_assemble(*drv_joints, switch_node, preserve_match_rotation = True, orient = 0.0)
    ...
    ensure_set(bind_hand)
    deco_mtx, wt_add_mtx = matrix_constraint([fk_joints[-1], ctrl_ik], bind_hand, r = True)
    ...
    ensure_group(hand_slide_grp, SHOW)
    posi_node, _, _, _ = connect_rivet(hand_slide_grp, ribbon_assemble_shape, 1, 'v')
    ...
    ensure_group(loc_hand, LOCATORS)
    _, wt_add_mtx = matrix_constraint([fk_joints[-1], ctrl_ik], loc_hand, t = True, r = True)
    reverse_node = cmds.createNode('reverse', name = f'rev_{loc_hand}')
    cmds.connectAttr(f'{switch_node}.switch', f'{reverse_node}.inputX')
    cmds.connectAttr(f'{reverse_node}.outputX', f'{wt_add_mtx}.wtMatrix[0].weightIn')
    cmds.connectAttr(f'{switch_node}.switch', f'{wt_add_mtx}.wtMatrix[1].weightIn')

    no_roll_locs(loc_hand, drv_joints[1], ctrl_end_b)

    # stretch -------------------------------------------------------------------------------------------------------
    drv_move = cmds.listRelatives(drv_joints[0], parent = True)[0]
    stretch_limb(ctrl_ik, drv_move, 'ctrl_main', drv_joints)
...
def create_clavicle(proxy_clavicle_jnt: str) -> tuple[str, str]:
    ...
    SIDE = get_side_from_node(proxy_clavicle_jnt)
    cmds.select(clear = True)
    ...
    ensure_set([bind_clavicle, bind_clavicle_end])
    ...
    color_node([bind_clavicle, bind_clavicle_end], 'white')
    ensure_group(bind_clavicle, JOINTS)
    move_op_matrix(bind_clavicle)

    ctrl = octagon_control(name = f'{CTRL}_clavicle_{SIDE}', normal = 'x', color = SIDE_COLOR[SIDE])
    ...
    matrix_constraint(ctrl, f'{bind_clavicle}_{MOVE}', t = True, r = True, mo = True)
    cb_attributes(ctrl, ats = ['sx', 'sy', 'sz'], lock = True, hide = True)
    sep_cb(ctrl, True)
...
def create_bind_hand(proxy_wrist_jnt: str):
    ...
    SIDE = get_side_from_node(proxy_wrist_jnt)
    ...
    color_node(bind_hand, 'white')
    ensure_group(bind_hand, JOINTS)
    move_op_matrix(bind_hand)
...
def create_bind_hand_02(proxy_wrist_jnt: str):
    ...
    color_node(bind_hand, 'white')
    ensure_group(bind_hand, SHOW)
    offset(bind_hand)

    return bind_hand, f'{bind_hand}_{OFFSET}'

def create_clavicle_arm_setup(proxy_clavicle_jnt: str, proxy_arm_jnt: str):
    '''
    '''
    ...
    fk_move, drv_move, ctrl_ik, pv, ctrl_start_a = create_ik_fk_setup_arm(proxy_arm_jnt)

    deco_mtx_fk_move = matrix_constraint(bind_clavicle, fk_move, t = True, r = True, mo = True)
    deco_mtx_drv_move = matrix_constraint(bind_clavicle, drv_move, t = True, r = True, mo = True)
    ...
    no_roll_locs(drv_start, bind_clavicle, ctrl_start_a, invert = True)
    ...
    _, wt_add_mtx_ctrl_ik_move = matrix_constraint([locator_world, ctrl_clavicle], ctrl_ik_move, t = True, r = True, mo = True)
    _, wt_add_mtx_pv_move = matrix_constraint([locator_world, ctrl_clavicle], pv_move, t = True, r = True, mo = True)
...
def create_clavicles_arms_setup(proxy_clavicle_jnt_l: str, proxy_arm_jnt_l: str, proxy_clavicle_jnt_r: str, proxy_arm_jnt_r: str):

    create_clavicle_arm_setup(proxy_clavicle_jnt_l, proxy_arm_jnt_l)
    create_clavicle_arm_setup(proxy_clavicle_jnt_r, proxy_arm_jnt_r)
