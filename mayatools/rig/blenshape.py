...
def transfer_deformation_to_blendshape(
    ...
    for control, transforms in controlers.items():
        ...
        for axis, values in transforms.items():
            ...
            if len(values) == 2:
                v_min, v_max = values
                transfer(base_geo, deformed_geo, blendshape_node, control, axis, v_min, suffix = '_min')
                transfer(base_geo, deformed_geo, blendshape_node, control, axis, v_max, suffix = '_max')
...
class TransferDialog(QDialog):
    ...
    def create_widgets(self):
        ...
        self.cb_tuple = (self.tx_cb, self.ty_cb, self.tz_cb, self.rx_cb, self.ry_cb, self.rz_cb, self.sx_cb, self.sy_cb, self.sz_cb)
        self.min_tuple = (self.tx_min, self.ty_min, self.tz_min, self.rx_min, self.ry_min, self.rz_min, self.sx_min, self.sy_min, self.sz_min)
        self.max_tuple = (self.tx_max, self.ty_max, self.tz_max, self.rx_max, self.ry_max, self.rz_max, self.sx_max, self.sy_max, self.sz_max)
    ...
    def run(self):
        ...
        transfer_deformation_to_blendshape(base_geo, deformed_geo, blendshape_node, controller_dict)
