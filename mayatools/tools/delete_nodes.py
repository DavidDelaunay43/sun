...
def delete_color_sets():
    ...
    for color_set in color_set_nodes:
        ...
        try:
            ...
            source_co = cmds.listConnections(color_set, source = True, destination = False, plugs = True)[0]
            ...
            if cmds.nodeType(source_co_node) == 'createColorSet':
                ...
                geo_input_co = cmds.listConnections(last_color_set, source = False, destination = True, plugs = True)[0]
                skincluster_output_co = cmds.listConnections(first_color_set, source = True, destination = False, plugs = True)[0]
                

            else: # cmds.nodeType(source_co_node) == 'skinCluster'
                first_color_set = color_set
                skincluster_output_co = cmds.listConnections(first_color_set, source = True, destination = False, plugs = True)[0]
                last_color_set = cmds.listConnections(first_color_set, source = False, destination = True, plugs = True)[0].split('.')[0]
                geo_input_co = cmds.listConnections(last_color_set, source = False, destination = True, plugs = True)[0]
