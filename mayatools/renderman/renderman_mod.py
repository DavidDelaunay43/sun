...
def connect_texture(pxr_surface_node: str, attribute_name: str, linearize: int, grader_cond: bool, texture: str = ''):
    '''
    '''
    ...
    pxr_texture_node = cmds.shadingNode('PxrTexture', asTexture = True, name = f'PTex_{NAME}_{attribute_name}')
    cmds.setAttr(f'{pxr_texture_node}.linearize', linearize)
    try:
        attribute_type = cmds.attributeQuery(attribute_name, node = pxr_surface_node, attributeType = True)
    ...
    if not grader_cond:
        if not attribute_name == 'displace':
            cmds.connectAttr(f'{pxr_texture_node}.{ptex_attribute}', f'{pxr_surface_node}.{attribute_name}')
    
    else:
        grader = REMAP_DICT['Color' in attribute_name] # créer une HSL ou un Remap ?
        grader_node = cmds.shadingNode(f'Pxr{grader}', asTexture = True, name = f'P{grader}_{NAME}_{attribute_name}')
        ...
        cmds.connectAttr(f'{pxr_texture_node}.{ptex_attribute}', f'{grader_node}.{grader_input_attribute}')
        try:
            cmds.connectAttr(f'{grader_node}.{grader_output_attribute}', f'{pxr_surface_node}.{attribute_name}')
        except: 
            pass
    ...
    texture_file_path = find_texture(file_name = current_file_name, lobe = attribute_name, texture = texture)
    ...
    if attribute_name == 'bumpNormal':
        bump_node = cmds.shadingNode('PxrBump', asTexture = True, name = f'PBump_{NAME}_{attribute_name}')
        cmds.connectAttr(f'{bump_node}.resultN', f'{pxr_surface_node}.{attribute_name}', force = True)
        ...
        print('displace')
        disp_transform_node = cmds.shadingNode('PxrDispTransform', asTexture = True, name = f'PDispTransform_{NAME}_{attribute_name}')
        displace_node = cmds.shadingNode('PxrDisplace', asTexture = True, name = f'PxrDisplace_{NAME}_{attribute_name}')
        shading_group_node = f'{pxr_surface_node}_SG'
        cmds.connectAttr(f'{disp_transform_node}.resultF', f'{displace_node}.dispScalar')
        cmds.connectAttr(f'{displace_node}.outColor', f'{shading_group_node}.rman__displacement')

        if not grader_cond:
            cmds.connectAttr(f'{pxr_texture_node}.resultRGBR', f'{disp_transform_node}.dispScalar')

        else:
            cmds.connectAttr(f'{grader_node}.resultRGBR', f'{disp_transform_node}.dispScalar')
...
def find_texture(file_name: str, lobe: str, texture: str = '', ext = '.png'):
    ...
    texture_file_name = '_'.join([prefix, asset_type, f'{asset_name}{texture}', lobe, f'ACES - ACEScg{ext}'])

    PROJECT_PATH = r'\\GANDALF\3d4_23_24\COUPDESOLEIL'
    TEXTURE_DIRNAME = '10_texture'
    ASSET_TYPE_DIRNAME =  asset_type_dict[asset_type]

    texture_directory = os.path.join(PROJECT_PATH, TEXTURE_DIRNAME, ASSET_TYPE_DIRNAME, asset_name)

    return os.path.join(texture_directory, texture_file_name)
        
def pxr_material(geo: Union[list, str], name: str, attributes: list, linearizes: list, graders: list):
    """
    """
